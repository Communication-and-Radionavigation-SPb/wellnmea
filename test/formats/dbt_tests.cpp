...
#include <wellnmea/formats/dbt.hpp>
...
TEST(DBT_constructor, can_be_instantiated) {
  EXPECT_NO_THROW({ DBT(); });
}

#define Suite DBT_Tests

class DBT_Tests : public ::testing::TestWithParam<json> {
 public:
  DBT format;
...
TEST_P(Suite, extracts_correct_values) {
  ...
  EXPECT_THAT(feets->as<wellnmea::instructions::DistanceValue>()->raw().value(),
              testing::DoubleNear(param["feet"], 0.01));
...
INSTANTIATE_TEST_SUITE_P(
    Container, Suite, ::testing::ValuesIn(getDataFromFile("dbt_data.json")),
    [](const ::testing::TestParamInfo<DBT_Tests::ParamType>& info) {
      std::stringstream ss;
      ss << "DBT_" << info.index;
